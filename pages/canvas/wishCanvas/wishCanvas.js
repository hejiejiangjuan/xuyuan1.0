...
Page({
  ...
  onLoad: function (options) {
    ...
    var linheight = item.activity_wish_list.length * 15
    this.setData({
  ...
  drawInit: function (list) {
    ...
    // //画背景
    // ctx.setFillStyle('#f4f4f4')
    // ctx.fillRect(0, currentLineHeight + imgHeight + ewimgHeight + Rpx * 60, canvasWidth, Rpx * 35)

    // // 尾部
    // let quanbu = '已有' + list.user_join_activity.length + '人参与'
    // let D = canvasWidth;
    // ctx.setFontSize(18);
    // ctx.setFillStyle('#576b95');
    // // 剩余宽度50%开始绘制文字
    // ctx.fillText(quanbu, (D - ctx.measureText(quanbu).width) * 0.5, currentLineHeight + imgHeight + ewimgHeight + Rpx * 80);
